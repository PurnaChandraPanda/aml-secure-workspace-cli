...
| AzureML secure workspace with customer-managed VNet | [bicep/azureml-secure-byon](./bicep/azureml-secure-byon) | [terraform/azureml-secure-byon](./terraform/azureml-secure-byon) | You need a private AzureML workspace that uses private endpoints and Private DNS in your own VNet without enabling an Azure ML managed VNet or deploying Azure Monitor Private Link Scope (AMPLS). |
...
- For secure AzureML infrastructure using private endpoints in a customer-managed VNet, start with `azureml-secure-byon`.
