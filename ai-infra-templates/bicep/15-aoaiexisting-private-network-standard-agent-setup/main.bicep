...
@description('Resource ID of the existing Azure OpenAI account')
param existingAoaiResourceId string

@description('Name for the project-scoped AOAI connection (string, must be unique within the project)')
...
// Create AOAI connection under Foundry Project
module addAoaiConn 'modules-network-secured/add-aoai-connection.bicep' = if(!empty(existingAoaiResourceId)) {
