...
@description('If true, do NOT create subnets; read them from an existing VNet instead.')
param useExistingSubnets bool = true
...
// Reference the existing virtual network agent subnet
resource existingAgentSubnet 'Microsoft.Network/virtualNetworks/subnets@2024-05-01' existing = if(useExistingSubnets) {
  parent: existingVNet
  name: agentSubnetName
}

// Reference the existing virtual network PE subnet
resource existingPESubnet 'Microsoft.Network/virtualNetworks/subnets@2024-05-01' existing = if(useExistingSubnets) {
  parent: existingVNet
  name: peSubnetName
}
