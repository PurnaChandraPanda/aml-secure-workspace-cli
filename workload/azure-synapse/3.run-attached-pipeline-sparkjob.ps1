...
$PipelineYamlPath=".\spark\attached-spark-pipeline.yml" # Path to the pipeline yaml file
...
az ml job create `
        --file $PipelineYamlPath --set "jobs.spark_job.compute=azureml:$AttachedSparkComputeName" `
        --workspace-name $WORKSPACE_NAME `
        --resource-group $RESOURCE_GROUP `
